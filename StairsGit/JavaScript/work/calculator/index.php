<?
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/header.php");
$APPLICATION->SetTitle("");
?>

<a href='/'> На главную</a>
<h1> Расчеты лестниц</h1>

<table class="tab_2"><tbody>

<tr><th>Тип лестницы</th><th>Расчет цены</th><th>Изготовление</th></tr>
	<tr>
		<td class="left">Тетивы/косоуры из листа</td>
		<td><a href="/calculator/metal/" target="_blank">Один этаж</a><br><a href="/calculator/multi?multiCalcType=metal" target="_blank">Много этажей</a></td>
		<td><a href="/manufacturing/metal/" target="_blank">Один этаж</a><br><a href="/manufacturing/multi?multiCalcType=metal" target="_blank">Много этажей</a></td>
	</tr>

	<tr>
		<td class="left">Монокосоуры</td>
		<td><a href="/calculator/mono/" target="_blank">Один этаж</a><br><a href="/calculator/multi?multiCalcType=mono" target="_blank">Много этажей</a></td>
		<td><a href="/manufacturing/mono/" target="_blank">Один этаж</a><br><a href="/manufacturing/multi?multiCalcType=mono" target="_blank">Много этажей</a></td>
	</tr>
	<tr>
		<td class="left">Винтовые</td>
		<td><a href="/calculator/vint/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/vint/" target="_blank">Открыть</a></td>
	</tr>
	<tr>
		<td class="left">Входные</td>
		<td><a href="/calculator/vhod/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/vhod/" target="_blank">Открыть</a></td>
	</tr>

	<tr>
		<td class="left">Больцевые</td>
		<td><a href="/calculator/bolz/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/bolz/" target="_blank">Открыть</a></td>
	</tr>
	<tr>
		<td class="left">Консольные</td>
		<td><a href="/calculator/console/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/console/" target="_blank">Открыть</a></td>
	</tr>

	<tr>
		<td class="left">Деревянные эконом</td>
		<td><a href="/calculator/timber_stock/" target="_blank">Один этаж</a><br><a href="/calculator/multi?multiCalcType=timber_stock" target="_blank">Много этажей</a></td>
		<td><a href="/manufacturing/timber_stock/" target="_blank">Один этаж</a><br><a href="/manufacturing/multi?multiCalcType=timber_stock" target="_blank">Много этажей</a></td>
	</tr>
	
	<tr>
		<td class="left">Вертикальные пожарки 2.0</td>
		<td><a href="/calculator/fire_2/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/fire_2/" target="_blank">Открыть</a></td>
	</tr>
	
	<tr>
		<td class="left">Ограждения</td>
		<td><a href="/calculator/railing/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/railing/" target="_blank">Открыть</a></td>
	</tr>

	<tr>
		<td class="left">Нестандарт</td>
		<td><a href="/calculator/custom/" target="_blank">Открыть</a></td>
		<td></td>
	</tr>
	
	<tr>
		<td class="left">Столешницы, слэбы</td>
		<td><a href="/calculator/slabs/" target="_blank">Открыть</a></td>
		<td></td>
	</tr>
	
	<tr>
		<td class="left">Навесы</td>
		<td><a href="/calculator/carport/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/carport/" target="_blank">Открыть</a></td>
	</tr>
	
	<tr>
		<td class="left">Веранды</td>
		<td><a href="/calculator/veranda/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/veranda/" target="_blank">Открыть</a></td>
	</tr>

	<tr>
		<td class="left">Мебель</td>
		<td><a href="/calculator/objects/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/objects/" target="_blank">Открыть</a></td>
	</tr>
	
	<tr>
		<td class="left">Шкафы</td>
		<td><a href="/calculator/coupe/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/coupe/" target="_blank">Открыть</a></td>
	</tr>
	
	<tr>
		<td class="left">Полностью деревянные (архив)</td>
		<td><a href="/calculator/timber/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/timber/" target="_blank">Открыть</a></td>
	</tr>
	
	<tr>
		<td class="left">Модульные (архив)</td>
		<td><a href="/calculator/module/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/module/" target="_blank">Открыть</a></td>
	</tr>
	<tr>
		<td class="left">Шкафы под лестницу (архив)</td>
		<td><a href="/calculator/wardrobe/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/wardrobe/" target="_blank">Открыть</a></td>
	</tr>

	<tr>
		<td class="left">Комоды (архив)</td>
		<td><a href="/calculator/sideboard/" target="_blank">Открыть</a></td>
		<td><a href="/manufacturing/sideboard/" target="_blank">Открыть</a></td>
	</tr>
	
	<tr>
		<td class="left">Столы(архив)</td>
		<td><a href="/calculator/table/" target="_blank">Открыть</a></td>
		<td></td>
	</tr>

	
</tbody></table>

<h2>Другие расчеты</h2>
<ul> 
	<li><a href="/manufacturing/strength_calc/" target="_blank">Расчет прогиба</a></li>
	<li><a href="/cnc/nesting/" target="_blank">Расчет количества листов</a></li>
	
	<li><a href="/calculator/geometry/" target="_blank">Калькулятор геометрии</a></li>
	<li><a href="/calculator/quick_calc/" target="_blank">Быстрый расчет</a></li>	
	<li><a href="/orders/calcs/" target="_blank">Поиск заказов в базе</a></li>
	<li><a href="/calculator/offers/stat/" target="_blank">Статистика расчетов</a></li>
	<li><a href="/calculator/carving/" target="_blank">Конфигуратор резьбы</a></li>
	<li><a href="/cnc/" target="_blank">Генераторы G-кода</a></li>	
	<li><a href="/bugs/" target="_blank">Багтрекер</a></li>
	<li><a href="/orders/orders/" target="_blank">Объекты в работе</a></li>
</ul>


<h2>Старые калькуляторы:</h2>

<ul> 
	<li><a href="/manufacturing/vhod_old/" >Старый калькулятор спецификации входной лестницы</a></li>
	<li><a href="/calculator/geometry_old/" >Старый калькулятор геометрии</a></li>
	<li><a href="/calculator/complect/entrance.php" >Спецификация на входную лестницу</a></li>
	<li><a href="/calculator/metal_old/" target="_blank">Старый ЛТ-КО (ноль у первой ступени)</a></li>
	<li><a href="/calculator/metal_old/" >Старый единый калькулятор цены</a></li>
	<li><a href="/calculator/geometry_old/straight.php" >Геометрия - прямая</a></li> 
	<li><a href="/calculator/geometry_old/turn_90.php" >Геометрия - Г-образная</a></li> 
	<li><a href="/calculator/geometry_old/turn_180.php" >Геометрия - П-образная двухмаршевая</a></li>
	<li><a href="/calculator/geometry_old/3_marsh.php" >Геометрия - П-образная трехмаршевая</a></li>
	<li><a href="/calculator/geometry_old/3d/" >Конфигуратор лестницы 3D</a></li> 
	<li><a href="/calculator/price/wood.php" >Расчет цены деревянной лестницы</a></li>
	<li><a href="/calculator/complect/ko.php" >Спецификация КО</a></li>
	<li><a href="/calculator/complect/lt.php" >Спецификация ЛТ</a></li>
	<li><a href="/calculator/dxf-generator.php" >Тестовый генератор dxf</a></li>
	<li><a href="/calculator/price/price_calc_1.6.php" >Старый калькулятор цены ЛТ и КО v.1.6</a></li>
	<li><a href="/calculator/test_calc.php" >Рисование на php - форма</a></li>
	<li><a href="/calculator/draw.php" >Рисование на php - обработчик</a></li>
	<li><a href="/calculator/price/vint.php" >Винтовая</a></li>
	<li><a href="/calculator/price/list.php" >Марш на тетивах из листа</a></li>
	<li><a href="/calculator/price/p-1.php" >Пожарки вертикальные - в разработке</a></li>
	<li><a href="/calculator/price/vhod.php" >Входные - в разработке</a></li>
	<li><a href="/calculator/mk/straight_mk.php" >МК прямая (МК-1) для цеха (в разработке)</a></li>
	<li><a href="/calculator/ml.php" >Марш лестниц МЛ для цеха</a></li>
	<li><a href="/calculator/ml_platform.php" >Площадка лестниц МЛ</a> - в разработке </li>
 </ul>
<!-- 
<h2>Структура калькулятора</h2>
  <li>Объявление общих переменных</li>
  <li>Объявление переменных для рабочих чертежей</li>
  <li>Объявление переменных для спецификации</li>
  <li>Получение данных из формы</li>
  <li>Задание констант для данной модели</li>
  <li>Расчет общих параметров лестницы</li>
  <li>Расчет размеров и выбор типов деталей для изготовления лестницы</li> 
  <li>Расчет количества деталей для спецификации</li>
</ul>
--><?require($_SERVER["DOCUMENT_ROOT"]."/bitrix/footer.php");?>