...
function drawConsoleFrames(par) {
	...
	if (marshPar.topTurn == 'площадка') {
		...
		for (var i = 0; i < 2; i++) {
			...
			if (framePar.type == 'platformTopP') {
				...
				if (params.stringerModel == 'короб') frame.position.z += 80 * turnFactor;
			}
			par.mesh.add(frame);
		}
	}

	if (marshPar.botTurn == 'площадка1') {
		if (params.stairModel == "П-образная с площадкой") {
			framePar.type = 'platformBotP';
			calcStringerConsoleFlanPar(framePar);
			for (var i = 0; i < 2; i++) {
				//flanPar.dxfBasePoint = newPoint_xy(flanPar.dxfBasePoint, 0, flanPar.widthTread + 50);
				var frame = drawConsoleFrame(framePar).mesh;
				frame.position.x = - framePar.widthTread * i - framePar.widthTread / 2;
				frame.position.y =  - params.treadThickness / 2 - framePar.heightBox / 2 * turnFactor;
				frame.rotation.z = Math.PI / 2;
				par.mesh.add(frame);


				//var flan = drawStringerConsoleFlan(flanPar);
				//flan.position.x = params.nose - offsetX - flanPar.widthFlan - flanPar.widthTread * i;
				//flan.position.y = flanPar.shiftFlanY;
				//par.mesh.add(flan);
			}
		}
	...
	if (params.stringerModel == 'короб') {
		if (marshPar.topTurn == 'забег') {
			...
			//var frame1 = drawMesh(meshPar).mesh;
			...
			//var frame2 = drawMesh(meshPar).mesh;
			...
			//var frame3 = drawMesh(meshPar).mesh;
			...
			frame3.position.x = posX + 80 + calcStringerMoove(par.marshId).stringerOutMooveNext;
			frame3.position.y = posY + marshPar.h * 2;
			frame3.position.z = (framePar.wndFrames['3'].p1Out.x + 80 + calcStringerMoove(par.marshId).stringerOutMoove) * turnFactor;
			...
			wndFrames.position.z = (-params.M / 2 - 80 - calcStringerMoove(par.marshId).stringerOutMoove)* turnFactor;
...
function calcStringerConsoleFlanPar11(par) {
...
function calcConsoleFrameWndPar(par) {
	...
	var stringerThickness = 80; //ширина тетивы
...
function calc99StringerConsoleFramePar11(par) {
	...
	calcStringerConsoleFlanPar(par)

	par.heightBox = params.treadThickness - par.offsetFrameY;
	par.widthBox = par.widthFlan - par.offsetFrameX * 2;
	par.lenghtBox = params.M + calcStringerMoove(par.marshId).stringerOutMoove - 20;
	...
	if (params.stringerModel == 'короб') {
		par.widthBox = par.widthFlan = par.widthTread - par.offsetFrameX * 2;
		if (~par.type.indexOf('platform')) {
			var marshFlanPar = calcStringerConsoleFlanPar({ marshId: par.marshId, type: 'marsh' });
			par.widthBox = par.widthFlan = marshFlanPar.widthTread - par.offsetFrameX * 2;
		}
		par.lenghtBox += 80;
	}
...
function calcConsoleFramePar(par) {
	...
	if (params.stringerModel == 'короб') {
		 par.lenghtBox += 80;
		 if (par.type == 'platformTopP') par.lenghtBox += 80;
	}
...
function drawConsoleFrame(par) {
	...
	if (!(~par.type.indexOf("winder") || par.type == 'platformBotP')) {
		...
		plates.position.z = thk;
		frame.add(plates);
		//plates.position.x = plateTop.position.x;
		...
		//	if (params.stairModel == "Прямая") {
		//		box.position.z = -lenghtBox;
		//		if (turnFactor == - 1) box.position.z = 0;
		//	}
		//}
	...
	if (~par.type.indexOf("winder")) {
		...
		if (par.type == "winder1" || par.type == "winder3") {
			...
			if (turnFactor == - 1) box.position.z = -calcStringerMoove(par.marshId).stringerOutMoove;
			if (params.stringerModel == 'короб') {
				box.position.z = calcStringerMoove(par.marshId).stringerOutMoove + 80 + 4;
				if (turnFactor == - 1) box.position.z = -calcStringerMoove(par.marshId).stringerOutMoove;
			}
			box.rotation.x = -Math.PI / 2 * turnFactor;
			box.rotation.z = -Math.PI / 2;
...
function drawConsoleEdge(par) {
	...
	if (!par.pDivide) {
		...
		if (par.angRotate) {
			...
			p2.y -= par.width * Math.tan(par.angRotate) * Math.tan(calcAngleX1(lines[1].p1, lines[1].p2))
		}
