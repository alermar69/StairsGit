	...
	if (typeof specObj != 'undefined') {
		if (!specObj[partName]) {
			specObj[partName] = {
				...
				area: 0,
				paintedArea: 0,
				name: "Площадка треугольная " + params.treadsMaterial,
				metalPaint: true,
				timberPaint: false,
				division: "metal",
				workUnitName: "area", //единица измерения
		...
		if (par.platformType == "square") specObj[partName].name = "Площадка прямоугольная " + params.treadsMaterial;
...
function drawRoundFlan(par) {
	...
	//сохраняем данные для спецификации
	if (par.partName == "topFlan" && (params.treadsMaterial == "рифленая сталь" || params.treadsMaterial == "лотки под плитку")) thk = 8;
	if (typeof specObj != 'undefined') {
...
function drawMidFix(par) {
	...
	*/
	...
	bal.position.y = 0;
	if (params.treadsMaterial == "рифленая сталь" || params.treadsMaterial == "лотки под плитку") bal.position.y = 40 + 8;
