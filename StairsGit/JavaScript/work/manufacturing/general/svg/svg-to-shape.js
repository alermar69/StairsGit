function transformSVGPath(pathStr) {
	...
	//преобразуем числа с плавающей точкой чтобы избавитсья от -e
	while(pathStr.indexOf("e") != -1){
		var pos = pathStr.indexOf("e");
		var endPos = pos;
		while(endPos < pathStr.length && pathStr[endPos] != " " && pathStr[endPos] != ","){
			endPos ++;
		}
		
		var startPos = endPos - 1;
		while(startPos > 0 && pathStr[startPos] != " " && pathStr[startPos] != ","){
			startPos --;
		}
		
		if(startPos > 1) {
			pathStr = pathStr.slice(0, startPos + 1) + "0" + pathStr.slice(endPos);
			//console.log(startPos, endPos, pathStr)
		}
	}
