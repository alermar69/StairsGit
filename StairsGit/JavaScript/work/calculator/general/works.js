...
function getWorkId(partId) {
	...
	if (treadType == "timber") {
		if (partId == "tread" || partId == "platformTread") {
			timber = "rect";
			painting = "plane";
		}
		if (partId == "wndTread" || partId == "wndTreadMid" || partId == "timberTurnTread") {
			timber = "winder";
			painting = "plane";
			cnc = "tread";
		}
...
function calcAssemblingWage() {
	...
	if (params.printALL) calcAll = true;



	if (params.printALL || params.calcType != "vint" && params.calcType != "objects") {
		...
		if (calcAll || params.calcType == "railing" && params.stairType != "нет" && params.isAssembling != "нет") {
			wages.treads.items = [];
			var costFactor = 1.5; //наценка 14.10.20

			//фанера
			if (calcAll || params.plywoodThk) {
				var wage = {
			...
			//шаблонирование
			if (calcAll || params.needMockup == "да") {
				var wage = {
		...
		if (params.calcType != "railing" && params.calcType != "carport" && params.calcType != "veranda" || calcAll)
	...
	if (calcAll || params.calcType == "vint") {
		//каркас, ступени
	...
	if (calcAll || params.calcType == "carport") {
		...
		//опоры столбов
		if (calcAll || params.fixType == "бетон") {
			var wage = {
		...
		if (calcAll || params.fixType == "винтовые сваи") {
			var wage = {
	...
	if (calcAll || params.calcType == "railing") {
		var railingTypes = [];
		var railingModels = []
		$("#railingType0 option").each(function () {
			railingModels.push(this.value);
		});
		if (calcAll) {
			$("#railingModel option").each(function () {
				railingModels.push(this.value);
			});
		}
		railingModels.forEach(function(railingModel){
			var railingType = {
				name: railingModel,
				meterCost: 700,
				amt: 0,
			};
			if (calcAll) railingType.amt = 1;
			if (railingType.name == "Стекло на стойках") railingType.meterCost = 1000;
		...
		$.each(railingTypes, function () {
			if (this.amt) {
				var wage = {
					name: "Монтаж ограждений " + this.name + " (бетон)",
	...
	var railingModels = [params.railingModel];
	if (calcAll) {
		railingModels = [];
		$("#railingModel option").each(function () {
			railingModels.push($(this).attr("value"))
		})
	}
	...
	if (railingModels) {
		railingModels.forEach(function(railingModel){
			var wage = {
				name: "Монтаж ограждений лестницы (" + railingModel + ")",
				amt: 0,
				unitName: "м.п.",
				unitWage: 350,
				total: 0,
			...
			if (railingModel == "Стекло на стойках") wage.unitWage = 500;
			if (railingModel == "Самонесущее стекло") wage.unitWage = 1000;
			if (railingModel == "Кованые балясины") wage.unitWage = 350;
			if (railingModel == "Деревянные балясины") wage.unitWage = 500;
			if (railingModel == "Стекло") wage.unitWage = 500;
			if (railingModel == "Дерево с ковкой") wage.unitWage = 500;
	...
	var railingModels = [params.railingModel_bal];
	if (calcAll) {
		railingModels = [];
		$("#railingModel_bal option").each(function () {
			railingModels.push($(this).attr("value"))
		})
	}
	...
	if (railingModels) {
		railingModels.forEach(function(railingModel){
			var wage = {
				name: "Монтаж балюстрады (" + railingModel + ")",
				amt: calcBanisterLen(), //функция в этом файле
				unitName: "м.п.",
				unitWage: 700,
				total: 0,
			}
			if (railingModel == "Стекло на стойках") wage.unitWage = 1000;
			if (railingModel == "Самонесущее стекло") wage.unitWage = 2000;
			if (railingModel == "Кованые балясины") wage.unitWage = 700;
	...
	if(params.minWage == "нет") minWage.base = 0;
