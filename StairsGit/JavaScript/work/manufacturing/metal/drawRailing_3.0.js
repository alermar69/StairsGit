...
function drawGlassSectionMetal(par) {
	...
	var nextMarshParams = getMarshParams(marshPar.nextMarshId);
	...
	if (!par.keyPoints.marshBotLineP1) par.keyPoints.marshBotLineP1 = {
		x: 0,
		y: 0
	};
	if (!par.keyPoints.marshBotLineP2) par.keyPoints.marshBotLineP2 = {
		x: 100,
		y: 100
	};
	...
	if (par.botEnd == "нет" && par.marshId !== 1) p0 = newPoint_xy(par.keyPoints.botLineP0, par.glassOffsetZ, 0);
	...
	if (par.botEnd != "нет") {
		...
		if (par.botConnection) glassPos = newPoint_xy(glassPos, -par.glassOffsetZ - stringerSideOffset, 0);
		...
		if (par.railingSide == 'left') glass.position.z = -par.glassOffsetZ - glassThickness;
		section.add(glass);
	...
	if (!marshPar.lastMarsh && par.topEnd == "нет") {
		marshSectLen = par.keyPoints.botLineP10.x - marshStart.x - par.glassOffsetZ - glassThickness - glassDist;
		if (params.stairModel == "П-образная с забегом" || params.stairModel == "П-образная с площадкой"){
			marshSectLen += glassThickness + glassDist;
		}
		if (params.model == "ко") {
			marshSectLen -= params.sideOverHang;
			if (params.stairModel == "П-образная с площадкой") marshSectLen += 45;
		}
		//учитываем вертикальный поручень на повороте
		marshSectLen -= (meterHandrailPar.profZ - glassThickness) / 2
	...
	if (par.isRearPWnd || par.isRearPRailing) {
		var glassPos = newPoint_xy(par.keyPoints.botLineP0, -stringerSideOffset + glassDist - par.glassOffsetZ - params.stringerThickness, 0);
		...
		var totalSectLen = par.keyPoints.botLineP10.x - par.keyPoints.botLineP0.x - glassDist +
			params.stringerThickness + par.glassOffsetZ * 2 + glassThickness + stringerSideOffset * 2;
	...
	if (par.isPlatform) {
		...
		//glassPos.x -= par.glassOffsetZ - glassDist + stringerSideOffset;
		marshAngle = 0;
		glassLengths = [];

		//var totalSectLen = par.keyPoints.botLineP10.x;
		var totalSectLen = par.keyPoints.botLineP10.x - par.keyPoints.botLineP0.x;
		//totalSectLen += par.glassOffsetZ * 2 + glassThickness + stringerSideOffset * 2;// - glassDist;
		...
		*/
		...
		if (par.marshPar.topTurn == 'пол' && i == (glassLengths.length - 1)) {
			...
			if (params.topGlassExtraLength > 0) {
				...
				glassPar.overlapCutHeight = glassHeight - par.marshPar.h - stringerParams.stringerWidth / Math.cos(marshPar.ang) - 10;
			}
	...
	if (par.topEnd != "нет") {
		...
		glassPos = newPoint_xy(glassPos, glassDist, 0);
		var extraLenX = stringerSideOffset - glassDist;
		if (par.topConnection) extraLenX += par.glassOffsetZ + glassDist + glassThickness;
	...
	if (typeof anglesHasBolts != "undefined" && anglesHasBolts && par.showPins) { //anglesHasBolts - глобальная переменная
		...
		var rutelMaterial = new THREE.MeshLambertMaterial({
			color: 0xFF0000
		});
		...
		for (var i = 0; i < par.racks.length; ++i) {
			...
			rutel.position.z = -rutelLength / 2 + glassThickness + par.glassOffsetZ;
			if (par.railingSide == 'left') rutel.position.z = rutelLength / 2 - glassThickness - par.glassOffsetZ;
	...
	if (params.handrail !== "нет") {
		...
		if (marshPar.lastMarsh) {
			...
			if(params.topGlassExtraLength > 0) handrailParams.extraLengthEnd += params.topGlassExtraLength / Math.cos(marshPar.ang);
		}
		else{
		...
		if (params.handrailFixType == "паз") {
			handrail.position.z = -handrailParams.wallOffset - par.glassOffsetZ - glassThickness / 2;
			if (par.railingSide == "right") handrail.position.z = handrailParams.wallOffset + par.glassOffsetZ + glassThickness / 2;
		}
