class Wardrobe extends AdditionalObject {
	...
	constructor(par){
		...
		sectParams = drawWardrobeObj(sectParams);
...
function drawWardrobeObj(par){
	...
	var p0 = { x: 0, y: par.height - par.legsHeight };
	var p1 = itercection(p0, polar(p0, radAngle, 100), { x: par.width, y: 0 }, { x: par.width, y: 100 });
...
//наполнение
	...
	var doorP2 = p0;
	var doorP3 = p1;
	var doorP4 = { x: par.width, y: 0 };

	if (par.side == 'правый') {
		var doorP1 = { x: 0, y: 0 };
		var doorP2 = { x: 0, y: p1.y };
		var doorP3 = { x: par.width, y: par.height - par.legsHeight };
		var doorP4 = { x: par.width, y: 0 };
	}
